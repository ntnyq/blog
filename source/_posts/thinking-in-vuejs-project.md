...
第一次在**Vue**项目中遇到这样的问题。所有的新闻列表都是有新闻位置的，但是三方API有时候并不能提供图片，新闻项的图片字段为**NULL**。

那么，首先项到的解决方案就是，在图片加载失败的时候，选择展现默认图片，将其地址赋值给图片的**src**属性。如下：

``` html
<img :src="images.jpg" onerror="this.src='path_to_the_pic/pic.jpg'" />
```

但在实际应用中，发现图片地址是个不小的问题，尤其是项目在打包后，会存在于服务器的某个目录下。

并且我在使用上述用法时，起先一直失败。

开始，我怀疑是因为单双引号的问题，导致内联JS无法正常执行。

后来，经过多次替换括号类型等，发现并不能解决问题。

突然，恍然大悟。因为图片并不存在**SRC**属性，所以也便不可能触发图片的出错事件，('onerror')。

所以也根本无法触发失败事件。

解决：在项目对应的**Store**文件中，寻找对应付的**JS模块**。

将图片使用webpack打包，使用引入好的图片。

使用`map`方法，改变`actions`里，将里面的空内容，修改为默认的内容。
并判断将当前新闻项是否为空9.
